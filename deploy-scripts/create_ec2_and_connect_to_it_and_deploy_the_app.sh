...
EC2_ID=$(aws ec2 run-instances \
  ...
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=$1-$EPOCHSECONDS-$$}]' \
...
EC2_PUBLIC_IP=$(aws ec2 describe-instances --instance-ids $EC2_ID | jq -r '.Reservations[0].Instances[0].PublicIpAddress')

# Output instance details
echo "Instance ID: $EC2_ID"
echo "Public IP: $EC2_PUBLIC_IP"

# Wait for a few seconds to allow the instance to fully initialize
sleep 90
